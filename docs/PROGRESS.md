...
| M8 农场领域层 | ◐ | M8.1 时间系统 ☑;作物生长(M8.2)+ 库存/物品(M8.3)+ NPC 日程调度(M8.4)待续 |
...
3. **M8.2 作物生长**:消费 `TimeStep.daysCrossed`,实现作物生长阶段推进(种植→浇水→成熟→收获)、作物 JSON 配置、作物 Tool API。
4. 按里程碑实现并回写本文件。ToolAPI 主线(M6 接口 + M7 首个客户端反向验证 + M8.x 领域层消费)是本项目区别于普通学习引擎的核心特色。
