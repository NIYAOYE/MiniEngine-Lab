...
6. **M8.3 库存/物品**:定义 `Inventory`/`ItemConfig` 领域模型(JSON 驱动);`crop.harvest` 产出写入库存;inventory.get/add/remove Tool API。
7. **接线缺口补 Tool(可选)**:`scene.create_entity` 加 parent/transform 参数 或 `scene.set_parent` Tool(让前端种子可重建层级);`entity.set_name`(替代前端本地 labels)。
8. 按里程碑实现并回写本文件。ToolAPI 主线(M6 接口 + M7 首个客户端反向验证 + M8.x 领域层消费 + M9.1 HTTP 传输层)是本项目区别于普通学习引擎的核心特色。
