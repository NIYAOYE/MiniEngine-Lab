...
| M9+ 未来 | ☐ | 网页编辑器前端接线(mock→fetch);M8.3 库存;对话/配方数据驱动、2D 物理/碰撞、存档;未来 Agent/LLM 接入白名单 Tool |
...
5. **网页编辑器前端接线**:把 `tools/editor-frontend/src/lib/toolClient.ts` 中的 mock `invoke()`/`listTools()` 替换为 `fetch http://127.0.0.1:8080`,UI 组件零改动;验证作物/实体/时间三面板与真实引擎通信。
6. **M8.3 库存/物品**:定义 `Inventory`/`ItemConfig` 领域模型(JSON 驱动);`crop.harvest` 产出写入库存;inventory.get/add/remove Tool API。
7. 按里程碑实现并回写本文件。ToolAPI 主线(M6 接口 + M7 首个客户端反向验证 + M8.x 领域层消费 + M9.1 HTTP 传输层)是本项目区别于普通学习引擎的核心特色。
